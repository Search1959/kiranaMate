...
import { DailyStats, Product, Sale } from '../types';
...
export const ReportsView: React.FC<ReportsViewProps> = ({
  ...
  const exportReportCSV = () => {
    const csvContent = "data:text/csv;charset=utf-8," +
      "Report Metric,Value\n" +
      `Today Sales Total,₹${stats.todaySalesTotal}\n` +
      `Cash Sales,₹${stats.cashSales}\n` +
      `UPI Sales,₹${stats.upiSales}\n` +
      `Total Outstanding Udhaar,₹${stats.totalPendingUdhaar}\n` +
      `Today Udhaar Collection,₹${stats.todayCollection}\n` +
      `Today Expenses,₹${stats.todayExpenses}\n` +
      `Estimated Net Profit,₹${stats.estimatedProfitToday}\n`;
      ...
      <div className="bg-slate-900 text-white p-5 rounded-3xl space-y-4 shadow-xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          <div className="bg-slate-800 p-3 rounded-2xl border border-slate-700">
            ...
            <span className="text-lg font-black text-emerald-400">₹{stats.todaySalesTotal}</span>
          </div>

          <div className="bg-slate-800 p-3 rounded-2xl border border-slate-700">
            <span className="text-slate-400 text-[10px] block">Expenses Logged</span>
            <span className="text-lg font-black text-rose-400">₹{stats.todayExpenses}</span>
          </div>
          ...
          <div className="bg-amber-400 text-slate-950 p-3 rounded-2xl font-bold">
            ...
            <span className="text-xl font-black text-emerald-950">₹{stats.estimatedProfitToday}</span>
          </div>
        </div>
      </div>
          ...
          {topSellingProducts.map((p, idx) => (
            <div key={p.id} className="py-2.5 flex items-center justify-between text-xs">
              ...
              <div className="text-right">
                ...
                <span className="text-[10px] text-slate-500">₹{p.sellingPrice} each</span>
              </div>
            </div>
          ))}
        </div>
